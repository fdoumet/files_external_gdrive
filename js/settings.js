...
var MOUNT_OPTIONS_DROPDOWN_TEMPLATE =
	...
	'				<label for="mountOptionsEncrypt">{{t "files_external_gdrive" "Enable encryption"}}</label>'+
	'			</span>'+
	'		</li>'+
	'		<li class="optionRow">'+
	'			<span class="menuitem">'+
	'				<input id="mountOptionsPreviews" class="checkbox" name="previews" type="checkbox" value="true" checked="checked"/>'+
	'				<label for="mountOptionsPreviews">{{t "files_external_gdrive" "Enable previews"}}</label>'+
	'			</span>'+
	'		</li>'+
	'		<li class="optionRow">'+
	'			<span class="menuitem">'+
	'				<input id="mountOptionsSharing" class="checkbox" name="enable_sharing" type="checkbox" value="true"/>'+
	'				<label for="mountOptionsSharing">{{t "files_external_gdrive" "Enable sharing"}}</label>'+
	'			</span>'+
	'		</li>'+
	'		<li class="optionRow">'+
	'			<span class="menuitem icon-search">'+
	'				<label for="mountOptionsFilesystemCheck">{{t "files_external_gdrive" "Check for changes"}}</label>'+
	'				<select id="mountOptionsFilesystemCheck" name="filesystem_check_changes" data-type="int">'+
	'					<option value="0">{{t "files_external_gdrive" "Never"}}</option>'+
	'					<option value="1" selected="selected">{{t "files_external_gdrive" "Once every direct access"}}</option>'+
	'				</select>'+
	'			</span>'+
	'		</li>'+
	'		<li class="optionRow">'+
	...
	'				<label for="mountOptionsReadOnly">{{t "files_external_gdrive" "Read only"}}</label>' +
	'			</span>' +
	'		</li>' +
	'	</ul>'+
	'</div>';
...
function addSelect2 ($elements, userListLimit) {
	...
	$elements.select2({
		placeholder: t('files_external_gdrive', 'All users. Type to select user or group.'),
		allowClear: true,
		multiple: true,
		toggleSelect: true,
		...
		ajax: {
			url: OC.generateUrl('apps/files_external_gdrive/applicable'),
			dataType: 'json',
			quietMillis: 100,
		...
		formatSelection: function (element) {
			if (element.type === 'group') {
				return '<span title="'+escapeHTML(element.name)+'" class="group">'+escapeHTML(element.displayname+' '+t('files_external_gdrive', '(group)'))+'</span>';
			} else {
...
/**
 * @class OCA.GDrive.Settings.StorageConfig
 *
...
/**
 * @class OCA.GDrive.Settings.GlobalStorageConfig
 * @augments OCA.GDrive.Settings.StorageConfig
 *
...
GlobalStorageConfig.prototype = _.extend({}, StorageConfig.prototype,
	/** @lends OCA.GDrive.Settings.GlobalStorageConfig.prototype */ {
	_url: 'apps/files_external_gdrive/globalstorages',
...
/**
 * @class OCA.GDrive.Settings.UserStorageConfig
 * @augments OCA.GDrive.Settings.StorageConfig
 *
...
UserStorageConfig.prototype = _.extend({}, StorageConfig.prototype,
	/** @lends OCA.GDrive.Settings.UserStorageConfig.prototype */ {
	_url: 'apps/files_external_gdrive/userstorages'
});

/**
 * @class OCA.GDrive.Settings.UserGlobalStorageConfig
 * @augments OCA.GDrive.Settings.StorageConfig
 *
 * @classdesc User external storage config
 */
...
UserGlobalStorageConfig.prototype = _.extend({}, StorageConfig.prototype,
	...
	_url: 'apps/files_external_gdrive/userglobalstorages'
});

/**
 * @class OCA.GDrive.Settings.MountOptionsDropdown
 *
...
MountOptionsDropdown.prototype = {
	...
	show: function($container, mountOptions, visibleOptions) {
		...
		var $el = $(template({
			mountOptionsEncodingLabel: t('files_external_gdrive', 'Compatibility with Mac NFD encoding (slow)')
		}));
		this.$el = $el;
...
/**
 * @class OCA.GDrive.Settings.MountConfigListView
 *
...
MountConfigListView.prototype = _.extend({
	...
	initialize: function($el, options) {
		...
		if (this._isPersonal) {
			this._storageConfigClass = OCA.GDrive.Settings.UserStorageConfig;
		} else {
			this._storageConfigClass = OCA.GDrive.Settings.GlobalStorageConfig;
		}
	...
	loadStorages: function() {
		...
		if (this._isPersonal) {
			...
			$.ajax({
				type: 'GET',
				url: OC.generateUrl('apps/files_external_gdrive/userglobalstorages'),
				data: {'testOnly' : true},
				contentType: 'application/json',
				success: function(result) {
					var onCompletion = jQuery.Deferred();
					$.each(result, function(i, storageParams) {
						...
						if (isUserGlobal) {
							...
							$tr.find('.configuration').text(t('files_external_gdrive', 'Admin defined'));
						}
					});
					var mainForm = $('#files_external_gdrive');
	 ...
	 * @return {OCA.GDrive.StorageConfig} storage model instance
	 */
	...
	deleteStorageConfig: function($tr) {
		...
		OC.dialogs.confirm(t('files_external_gdrive', 'Are you sure you want to delete this external storage?', {
				storage: this.mountPoint
			}), t('files_external_gdrive', 'Delete storage?'), function(confirm) {
			if (confirm) {
...
$(document).ready(function() {
	var enabled = $('#files_external_gdrive').attr('data-encryption-enabled');
	...
	$allowUserMounting.bind('change', function() {
		...
		if (this.checked) {
			OCP.AppConfig.setValue('files_external_gdrive', 'allow_user_mounting', 'yes');
			...
			OCP.AppConfig.setValue('files_external_gdrive', 'allow_user_mounting', 'no');
			$('#userMountingBackends').addClass('hidden');
		}
		OC.msg.finishedSaving('#userMountingMsg', {status: 'success', data: {message: t('files_external_gdrive', 'Saved')}});
	});
	...
	$('input[name="allowUserMountingBackends\\[\\]"]').bind('change', function() {
		...
		OCP.AppConfig.setValue('files_external_gdrive', 'user_mounting_backends', userMountingBackends.join());
		OC.msg.finishedSaving('#userMountingMsg', {status: 'success', data: {message: t('files_external_gdrive', 'Saved')}});
	...
	$('#global_credentials').on('submit', function() {
		...
		$submit.val(t('files_external_gdrive', 'Saving...'));
		$.ajax({
			type: 'POST',
			contentType: 'application/json',
			data: JSON.stringify({
				...
				url: OC.generateUrl('apps/files_external_gdrive/globalcredentials'),
				dataType: 'json',
			success: function() {
				$submit.val(t('files_external_gdrive', 'Saved'));
				setTimeout(function(){
					$submit.val(t('files_external_gdrive', 'Save'));
				}, 2500);
			}
		});
		return false;
	...
	// global instance
	OCA.GDrive.Settings.mountConfig = mountConfigListView;

	/**
	 * Legacy
	 *
	 * @namespace
	 * @deprecated use OCA.GDrive.Settings.mountConfig instead
	 */
	OC.MountConfig = {
...
OCA.GDrive = OCA.GDrive || {};
/**
 * @namespace
 */
OCA.GDrive.Settings = OCA.GDrive.Settings || {};

OCA.GDrive.Settings.GlobalStorageConfig = GlobalStorageConfig;
OCA.GDrive.Settings.UserStorageConfig = UserStorageConfig;
OCA.GDrive.Settings.MountConfigListView = MountConfigListView;
